---
title: iOS Bookmarks
date: 2020-01-17 13:47
updated: 2020-03-01 19:00
category: iOS
tag:
  - ios
  - swift
  - fastlane
  - test-automation
layout: page
---

# iOS Bookmarks

- [General](#general)
- [Swift](#swift)
- [Fastlane](#fastlane)
- [Automated Testing](#automated-testing)
  - [Launch Arguments](#launch-arguments)

- - -

## General

* [xcode-select active developer directory error](https://stackoverflow.com/questions/17980759/xcode-select-active-developer-directory-error)
* [What is the bundle identifier of apple's default applications in iOS?](https://stackoverflow.com/questions/9910366/what-is-the-bundle-identifier-of-apples-default-applications-in-ios)
* [iOS: Using UIDeviceOrientation to Determine Orientation](https://happyteamlabs.com/blog/ios-using-uideviceorientation-to-determine-orientation/)

## Swift

* [A Swift Tour](https://docs.swift.org/swift-book/GuidedTour/GuidedTour.html)
* [A Beginner’s Guide to Scripting in Swift - Capital One Tech - Medium](https://medium.com/capital-one-tech/a-beginners-guide-to-scripting-in-swift-aeda4dd3e67)

## Fastlane

* [Fastlane examples](https://github.com/fastlane/examples)
    * https://github.com/fastlane/examples/blob/master/Timehop/Fastfile
* [Directory behavior - fastlane - fastlane docs](https://docs.fastlane.tools/advanced/fastlane/#directory-behavior)

## Automated Testing

* [XCTest UI Testing Hints and Tips | Average Pro](http://averagepro.com/2015/11/06/xctest-ui-testing-hints-and-tips/)
* [Everything About Xcode UI Testing | @samwize](https://samwize.com/2016/02/28/everything-about-xcode-ui-testing-snapshot/)
* [iOS Accessibility Tutorial: Getting Started | raywenderlich.com](https://www.raywenderlich.com/845-ios-accessibility-tutorial-getting-started)
* [Accessibility and UI Testing in iOS - Jonathan Chen - Medium](https://medium.com/@jchen_77520/accessibility-and-ui-testing-in-ios-3eb0822a17fb)
* [iOS UI tests for custom URL scheme | Alexander Ross](https://aross.se/posts/2018-06-17-ios-ui-tests-for-custom-url-scheme/)

### Launch Arguments

* [xcodebuild launch arguments - StartPage Search Results](https://www.startpage.com/do/dsearch?query=xcodebuild+launch+arguments&cat=web&pl=opensearch&language=english_uk)
* [Running Xcode tests from the terminal | mokacoding](https://www.mokacoding.com/blog/running-tests-from-the-terminal/)
* [Passing arguments to iOS tests with xcodebuild - Stack Overflow](https://stackoverflow.com/questions/40722756/passing-arguments-to-ios-tests-with-xcodebuild)
* [Launch Arguments & Environment Variables - NSHipster](https://nshipster.com/launch-arguments-and-environment-variables/)
* [UI Testing with `NSUserDefaults`](https://www.matrixprojects.net/p/ui-testing-with-user-defaults/)
